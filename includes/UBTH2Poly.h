...
#include <vector>
  ...
  */
  ...
  public:
    ...
    /*UBTH2Poly(const char *name, const char *title, std::map<double, std::vector<double>> BinMap, bool isVerticalBinningType)
    : TH2Poly();*/
    
    UBTH2Poly(const char *name, const char *title, std::map<double, std::vector<double>> Bin_edges, bool isVerticalBinningType)
    : TH2Poly()
  {
   if (fgDefaultSumw2) {this->Sumw2(); fNcells = kNOverflow;} 
   this->SetName(name);
   this->SetTitle(title);
	 for (auto it = Bin_edges.begin(); it != Bin_edges.end(); ++it) {
        ...
        if (nextIt != Bin_edges.end()) {
            ...
            if(isVerticalBinningType ==  true)
            { 
               this->AddBin(x1, y1, x2, y2);
            }
            else{ 
               this->AddBin(currentKey, Xvector.at(k), nextKey, Xvector.at(k+1));}
            }
        } else {
    ...
    TH1D* ProjectionY(const char *name, Int_t firstxbin, std::vector<int> & bin_numbers) const;
     ...
     /// Projects in X slices, firstxbin is the Y bin you want to select, bin_numbersis a returned vectors with the corresponding bin numbers

    TH1D* Projection(const char *name, std::vector<int>  Input_bin_numbers) const;


    TH1D* ProjectionY(const char *name, std::vector<int> Input_bin_numbers) const;
    ...
    ClassDef(UBTH2Poly, 1) // TH2 with polygonal bins (extended for MicroBooNE)
