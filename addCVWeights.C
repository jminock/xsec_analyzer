...
double MCmocktankEReco(double tanktrack){
	const double muonEdep = 1.992; //MeV/cm in water
	double tankE = muonEdep*tanktrack*100.; //MCEnergy in tank
	return tankE;
}

double MCMRDEReco(double mrdtrack, double mrdangle){
	auto MRDenergyvspenetration=new TF1("af","expo(0)+pol0(2)+([3]/([4]-x))",0,1.6);
	MRDenergyvspenetration->SetParameters(-3.62645, 3.75503, 2.68525, 3.59244, 1.66969);
	double dEdx = MRDenergyvspenetration->Eval(mrdangle);
	double mrdE = mrdtrack*dEdx;
	return mrdE;
}

void addCVWeights(){
	//Open file and trees
	TFile *f = new TFile("/exp/annie/data/users/jminock/standard_tank_ntuples/PhaseIITree_large_unweighted_ntuple.root","update");
//	TFile *f = new TFile("/exp/annie/data/users/jminock/standard_tank_ntuples/PhaseIITree_20k_ntuple.root");
//	TFile *f = new TFile("/exp/annie/app/users/jminock/ToolAnalysis/PhaseIITree_0.0.0.root");
...
//	tMRD->SetBranchAddress("MRDTrackLength",&MRDTrackLengthMRD);
	...
	const double EMIN = 600.;
	const double EMAX = 1200.;

	double muon_m = 105.66;
	TCV_weight->push_back(1.); //Fill CV weights with 1 for all events
...
//	for (Long64_t i = 0; i < 100; i++) {
		tTrig->GetEntry(i);
		if(i%20000 == 0) std::cout << i << std::endl;
		...
		int npi = hasPi0 + hasPiP + hasPiM;
		int npiC = hasPi0 + hasPiPC + hasPiMC;
		...
		double MRDtotE = 0;
		bool inEnergyRange = (mcmuonE > EMIN && mcmuonE < EMAX);
		bool forwardGoing = (std::cos(mcangle * M_PI / 180.) > 0.8);
		bool isKinematic = (inEnergyRange && forwardGoing);
		bool isTrueSelection = (inFV && isCC && ismuon && inMRD && (!hasVisNonMuon) && isKinematic);
	
/*		for(int j = 0; j < TCV_weight->size(); j++){
			std::cout << TCV_weight->at(j) << std::endl;
		}*/
/*		for(int j = 0; j < TCV_weight->size(); j++){
			std::cout << TCV_weight->at(j) << std::endl;
		}
*/
		mcfv = inFV ? 1 : 0;
		TFV->Fill();
/*		for(auto& x: *xsecweights){
			for(int i = 0; i < x.second.size(); i++){
				hxsec->Fill(x.second.at(i));
				std::cout << x.second.at(i) << std::endl;
			}	
		}
		for(auto& x: *fluxweights){
			for(int i = 0; i < x.second.size(); i++){
				hflux->Fill(x.second.at(i));
			}
		}
		std::cout << All_weight->size() << " " << All_weight->at(0) << std::endl;
...
*//*		if(isTrueSelection){
			if((rcsr > 0.2) && numMRDTracks == 1 && recoFV && (MRDStop->at(0)) && (julieenergy > EMIN) && (julieenergy < EMAX) && (juliecostheta > 0.8)){
			}
			if((rcsr > 0.2) && numMRDTracks == 1 && simplerecoFV && (MRDStop->at(0)) && (simpleenergy > EMIN) && (simpleenergy < EMAX) && (simplecostheta > 0.8)){
			}
		}

	
		//Julie
		if((rcsr > 0.2) && recoFV && (MRDStop->at(0)) && (julieenergy > EMIN) && (julieenergy < EMAX) && (juliecostheta > 0.8)){
		}
*/		//Reco cuts
/*		if((rcsr > 0.2) && (numMRDTracks == 1) && recoFV && (MRDStop->at(0)) && (julieenergy > EMIN) && (julieenergy < EMAX) && (juliecostheta > 0.8)){
			if(isTrueSelection) hmcE->Fill(julieenergy);
		}*/
	}
